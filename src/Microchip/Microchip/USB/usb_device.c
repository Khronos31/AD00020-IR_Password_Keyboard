 ...
 *******************************************************************/
    ...
    volatile BDT_ENTRY BDT[(USB_MAX_EP_NUMBER + 1) * 2] USB_FIXED_AT(0x200);
#elif (USB_PING_PONG_MODE == USB_PING_PONG__EP0_OUT_ONLY)
    volatile BDT_ENTRY BDT[((USB_MAX_EP_NUMBER + 1) * 2)+1] USB_FIXED_AT(0x200);
#elif (USB_PING_PONG_MODE == USB_PING_PONG__FULL_PING_PONG)
    volatile BDT_ENTRY BDT[(USB_MAX_EP_NUMBER + 1) * 4] USB_FIXED_AT(0x200);
#elif (USB_PING_PONG_MODE == USB_PING_PONG__ALL_BUT_EP0)
    volatile BDT_ENTRY BDT[((USB_MAX_EP_NUMBER + 1) * 4)-2] USB_FIXED_AT(0x200);
#else
 ...
 *******************************************************************/
volatile CTRL_TRF_SETUP SetupPkt USB_FIXED_AT(0x220);           // 8-byte only
volatile BYTE CtrlTrfData[USB_EP0_BUFF_SIZE] USB_FIXED_AT(0x228);
...
// Can provide compile time option to do software pingpong
    ...
    volatile unsigned char hid_report_out[HID_INT_OUT_EP_SIZE] USB_FIXED_AT(0x230);
    volatile unsigned char hid_report_in[HID_INT_IN_EP_SIZE] USB_FIXED_AT(0x231);
#endif
...
static void USBCtrlEPService(void)
{
    ...
    if((USTATcopy.Val & USTAT_EP0_PP_MASK) == USTAT_EP0_OUT_EVEN)
    {
        ...
        if(pBDTEntryEP0OutCurrent->STAT.pid.PID == PID_SETUP)
        {
...
static void USBCtrlTrfInHandler(void)
{
    ...
    lastDTS = pBDTEntryIn[0]->STAT.cpu.DTS;
       ...
       (USBDeviceState == CONFIGURED_STATE))
    {
        ...
        else
        {
			//Else the request must have been a CLEAR_FEATURE endpoint halt.
                ...
                //function.
                if((current_ep_data.bits.transfer_terminated != 0) || ((p->STAT.cpu.UOWN == 1) && (p->STAT.cpu.BSTALL == 0)))
                {
                    if(SetupPkt.EPDir == 0)
                    {
...
static void USBCtrlTrfRxService(void)
{
    ...
    if(outPipes[0].wCount.Val > 0)
    {
        pBDTEntryEP0OutNext->CNT = USB_EP0_BUFF_SIZE;
        pBDTEntryEP0OutNext->ADR = ConvertToPhysicalAddress(&CtrlTrfData);
        if(pBDTEntryEP0OutCurrent->STAT.cpu.DTS == 0)
        {
            pBDTEntryEP0OutNext->STAT.Val = _USIE|_DAT1|_DTSEN;
        }
        else
...
static void USBConfigureEndpoint(BYTE EPNum, BYTE direction)
{
    ...
    handle->STAT.cpu.UOWN = 0;  //mostly redundant, since USBStdSetCfgHandler() 
    ...
    else
    {
        ...
        handle->STAT.cpu.DTS = 0;
        (handle+1)->STAT.cpu.DTS = 1;
