...
#elif defined(COMPILER_MPLAB_XC8)
	#define PTR_BASE		unsigned short
	#define ROM_PTR_BASE	unsigned short
...
/* XC8 does not honor the C18 named-data-section pragmas used by the old
 * USB stack. The PIC18F14K50 USB SIE requires its BDT and transfer buffers
 * to live in the device's USB RAM, so provide a portable marker for the
 * XC8-specific absolute placement below. */
#if defined(COMPILER_MPLAB_XC8)
    #define USB_FIXED_AT(address) __at(address)
#else
    #define USB_FIXED_AT(address)
#endif
